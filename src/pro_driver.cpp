 ...
 **/
int FTDI_SendData(FT_HANDLE device_handle,int label, unsigned char *data, int length)
{
 ...
 * Parameters: Label, Pointer to Data Structure, Length of Data
 **/
int FTDI_ReceiveData(FT_HANDLE device_handle,int label, unsigned char *data, unsigned int expected_length)
{
 ...
 **/
uint16_t FTDI_OpenDevice(FT_HANDLE * device_handle,DMXUSBPROParamsType * PRO_Params,int device_num)
{
	int RTimeout =120;
	int WTimeout =100;
	int VersionMSB =0;
	...
	if (ftStatus == FT_OK) 
	{
		...
		printf("Sending GET_WIDGET_PARAMS packet... ");
 		res = FTDI_SendData(*device_handle,GET_WIDGET_PARAMS,(unsigned char *)&size,2);
		// Check Response
		if (res == NO_RESPONSE)
		{
			FT_Purge (*device_handle,FT_PURGE_TX);
		...
		res=FTDI_ReceiveData(*device_handle,GET_WIDGET_PARAMS_REPLY,(unsigned char * )PRO_Params,sizeof(DMXUSBPROParamsType));
		// Check Response
		if (res == NO_RESPONSE)
		{
		...
		res = FTDI_SendData(*device_handle,GET_WIDGET_SN,(unsigned char *)&size,2);
		res=FTDI_ReceiveData(*device_handle,GET_WIDGET_SN,(unsigned char *)&temp,4);
...
// Read a DMX packet
uint8_t FTDI_RxDMX(FT_HANDLE device_handle,uint8_t label, unsigned char *data, uint32_t* expected_length)
{
